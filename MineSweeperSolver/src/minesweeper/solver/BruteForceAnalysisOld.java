...
public class BruteForceAnalysisOld extends BruteForceAnalysisModel{
	...
	public BruteForceAnalysisOld(Solver solver, List<? extends Location> locations, int size, String scope, List<Location> startLocations) {
	...
	protected void process() {
		...
		for (LivingLocation move: top.getLivingLocations()) {
			...
			if (move.linked) {
				if (linkedPm != null) {
					solver.logger.log(Level.INFO, "Tile %s is living with %d values and probability %s, this location is linked with %s", locations.get(move.index), move.count, percentage(singleProb), linkedPm.location);
				}

			} else if (move.pruned) {
				solver.logger.log(Level.INFO, "Tile %s is living with %d values and probability %s, this location was pruned (max winning lines %d)", locations.get(move.index), move.count, percentage(singleProb),  winningLines);
			} else {
				solver.logger.log(Level.INFO, "Tile %s is living with %d values and probability %s, winning lines %d", locations.get(move.index), move.count, percentage(singleProb),  winningLines);
			}
