...
public class BruteForceAnalysis2 extends BruteForceAnalysisModel{
	
	/*
	// used to hold all the solutions left in the game
	private class SolutionTable1 {
		
		private final List<byte[]> solutions = new ArrayList<>();
		...
		private void sortSolutions(int start, int end, int index) {
			...
			Collections.sort(solutions.subList(start, end), new SortSolutions(index));
	...
	*/
	...
	public BruteForceAnalysis2(Solver solver, List<? extends Location> locations, int size) {
	...
	*/
	...
	protected Action getNextMove(BoardState boardState) {
		...
		while (boardState.isRevealed(loc)) {
			...
			//TODO fails here because bestLiving.children[value] is null
